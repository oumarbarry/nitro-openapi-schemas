...
| `@valibot/to-json-schema`'s plain `toJsonSchema()` predates Standard JSON Schema and returns a raw JSON Schema object, not a spec-compliant wrapper — silently produces schemas but breaks named-schema hoisting since there's no `~standard` to read | use `toStandardJsonSchema()` (1.5+) instead, and read `v.metadata({ id })` off the raw schema's `pipe` *before* wrapping, since the wrapper strips everything down to `~standard` |
