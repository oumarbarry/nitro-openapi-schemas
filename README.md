...
- [`src/index.ts`](src/index.ts): the Nitro module (~50 lines). Emits a virtual module that imports every scanned route handler directly (bypassing lazy wrappers), because h3's `defineValidatedHandler`/`defineHandler` call `Object.assign` on the definition, including live `validate` schemas and `meta`, onto the handler function. No AST extraction, no sandboxed evaluation.
...
- [`examples/nitro/`](examples/nitro/): payments API demo plus [`scripts/emit-openapi.mjs`](examples/nitro/scripts/emit-openapi.mjs), the build-artifact half. It boots the built server, snapshots the spec, and writes `openapi.json` (FastAPI-style "import the app and ask it").

## What this demonstrates for the RFC

1. No build-time magic is required for schema-driven OpenAPI: the schemas are already in the bundle, since they validate requests. The only piece missing from Nitro core is giving the OpenAPI route access to handler objects, instead of the statically-extracted `handlersMeta`.
